...
def compute_trajLf(max_steps, **kwargs):
    ...
    #     envl.env.force_mag = kwargs['force_mag']
        ...
        # for _ in range(max_steps):
            ...
            # obl, r, terminated, _=envl.step(action)
            reward = reward+r
...
def compute_trajMf(max_steps, **kwargs):
    ...
    #     envm.env.force_mag = kwargs['force_mag']
        ...
        # for _ in range(max_steps):
            ...
            action=pi
            # obm, r, terminated,_=envm.step(action)
...
class test_module:
    ...
    def run_BO(self, iters_BO):
        ...
        new_mf_c=MF_c + 56.41*(all_ce_mid+all_ce_low)
...
for r in rand_num2:
      ...
      node1_m = pred_node(f=lambda trajMf: pred5(trajMf))
      node2_m = pred_node(f=lambda trajMf: pred6(trajMf))
      node3_m = pred_node(f=lambda trajMf: pred7(trajMf))
      node4_mf = max_node(children= [node1_m, node3_m, node2_m])

      node1_h = pred_node(f=lambda trajHf: pred8(trajHf))
      node2_h = pred_node(f=lambda trajHf: pred9(trajHf))
      node3_h = pred_node(f=lambda trajHf: pred10(trajHf))
