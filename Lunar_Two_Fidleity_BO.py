...
class test_module:
    ...
    def initialize(self):
        ...
        # self.f_acqu=self.f_tree[0]
        # f_x_low=self.f_acqu.eval_robustness(trL)
    ...
    def run_BO(self, iters_BO):
        ...
        sum_L=(valid_low_ce)+(real_low_ce)
        sum_H=(valid_high_ce)+(real_high_ce)
...
def pred3(trajLf):
      traj = trajLf[0]
      # print(f" whole tr is {trajLf}")
      # print(f"traj zero is {traj}")
      for state in traj:
         theta=state[4]
...
for r in rand_num2:
      ...
      node2_h = pred_node(f=pred5)
      node4_hf = min_node(children=[node1_h,node2_h,node0_h])
      ...
      print(f"number of ces in HF: {sum_H}")
      print(f"number of ces in LF: {sum_L}")
